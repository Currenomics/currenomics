...
# Full Refresh
CREATE_TABLE_SQL = f"""
DROP TABLE {SNOWFLAKE_DATABASE}.{SNOWFLAKE_SCHEMA}.{SNOWFLAKE_TABLE};
...
def extract(**kwargs):
    ...
    unemployment_df = pd.DataFrame({"date": unemployment_data.index, "unemployment_rate": unemployment_data.values})
...
def transform(**kwargs):
    ...
    file_content = s3_hook.read_key(
        key=f"tmp/{ds_nodash}_{SNOWFLAKE_TABLE}_tmp.csv", bucket_name="pjt-currenomics"  # 임시로 저장
    )
...
create_table = SnowflakeOperator(
    task_id="create_table_if_not_exists", snowflake_conn_id="snowflake_conn_id", sql=CREATE_TABLE_SQL, dag=dag
)

# 4. S3의 정제 데이터를 Snowflake로 COPY 태스크
load = SnowflakeOperator(task_id="load_to_snowflake", snowflake_conn_id="snowflake_conn_id", sql=COPY_INTO_SQL, dag=dag)

# 4. S3의 임시파일 삭제
delete_temp_data = PythonOperator(task_id="delete", python_callable=delete, provide_context=True, dag=dag)
